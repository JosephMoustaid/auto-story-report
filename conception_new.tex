...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{images/chapter-4/uml diagramms/use-case-diagramm.png}
\caption{Diagramme de cas d'utilisation du système AutoStory}
\label{fig:use-case-diagram}
\end{figure}

\textbf{Acteurs principaux :}
\begin{itemize}
    \item \textbf{Utilisateur} : Interagit avec le système pour générer des contenus
    \item \textbf{Administrateur} : Gère le système et les utilisateurs
    \item \textbf{Google Gemini API} : Service externe de génération de contenu IA
\end{itemize}

\textbf{Cas d'utilisation :}
\begin{itemize}
    \item Rechercher des véhicules
    \item Consulter les détails d'un véhicule
    \item Générer une story AI
    \item Générer une vidéo promotionnelle
    \item Exporter en PDF
    \item Visualiser en 3D
    \item Comparer des véhicules
    \item Obtenir des recommandations
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/chapter-4/uml diagramms/classs-diagramm.jpg}
\caption{Diagramme de classes du système AutoStory}
...
\begin{itemize}
    \item \textbf{User} : Représente un utilisateur du système
    \begin{itemize}
        \item Attributs : id, name, email, password, role, createdAt
        \item Méthodes : register(), login(), updateProfile()
    \end{itemize}
    
    \item \textbf{Vehicle} : Modélise un véhicule automobile
    \begin{itemize}
        \item Attributs : id, make, model, year, engine, specifications
        \item Méthodes : getDetails(), getSpecs(), getImages()
    \end{itemize}
    
    \item \textbf{Story} : Représente une narrative générée par l'IA
    \begin{itemize}
        \item Attributs : id, vehicleId, content, tone, language
        \item Méthodes : generate(), export(), translate()
    \end{itemize}
    
    \item \textbf{Video} : Encapsule une vidéo générée
    \begin{itemize}
        \item Attributs : id, storyId, url, duration, format
        \item Méthodes : generate(), download(), stream()
    \end{itemize}
    
    \item \textbf{AIService} : Service d'intégration avec Gemini
    \begin{itemize}
        \item Méthodes : generateText(), analyzeImage(), chat()
    \end{itemize}
\end{itemize}
...
\subsubsection{Génération de story}

Le diagramme suivant détaille le processus de génération d'une narrative AI :

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/chapter-4/uml diagramms/generate-story-sequence-diagramm.png}
\caption{Diagramme de séquence - Génération de story}
\label{fig:sequence-story}
\end{figure}

\textbf{Flux d'exécution :}
\begin{enumerate}
    \item L'utilisateur sélectionne un véhicule et configure les paramètres (tone, language)
    \item Le frontend envoie une requête POST au backend
    \item Le backend récupère les données du véhicule depuis MongoDB
    \item Le service AI construit un prompt optimisé
    \item L'API Gemini génère la narrative
    \item Le backend traite et valide la réponse
    \item La story est sauvegardée en base de données
    \item Le résultat est retourné au frontend pour affichage
\end{enumerate}

\subsubsection{Génération de scène 3D}

Ce diagramme montre le processus de création d'une visualisation 3D :

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/chapter-4/uml diagramms/generate-3d-scene-sequence-diagramm.png}
\caption{Diagramme de séquence - Génération de scène 3D}
\label{fig:sequence-3d}
\end{figure}

\textbf{Étapes principales :}
\begin{enumerate}
    \item L'utilisateur demande la visualisation 3D d'un véhicule
    \item Le frontend charge Three.js et initialise le renderer
    \item Le loader récupère le modèle 3D (.glb/.gltf)
    \item La scène est construite avec lumières et caméras
    \item Les annotations interactives sont positionnées
    \item Le mode Ghost est configuré (vue transparente)
    \item La boucle de rendu démarre pour l'interaction utilisateur
    \item L'utilisateur peut manipuler la caméra et interagir avec les hotspots
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Backend Node.js/Express} : API REST construite avec Express.js, gérant la logique métier
    \item \textbf{Frontend React} : Interface utilisateur moderne développée en TypeScript
    \item \textbf{Services externes} : Google Gemini API, Wikipedia API, Puppeteer
    \item \textbf{Stockage} : MongoDB pour les données, système de fichiers pour les médias générés
\end{itemize}
...
\begin{itemize}
    \item Logging structuré (Winston/Morgan)
...
\begin{itemize}
    \item Traitement asynchrone des requêtes (async/await Node.js)
    \item Optimisation des images avant traitement
    \item Indexes MongoDB pour requêtes rapides
    \item Compression des médias générés
\end{itemize}
